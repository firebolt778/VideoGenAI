...
import { insertVideoTemplateSchema, type InsertVideoTemplate } from "@shared/schema";
...
export default function EnhancedStoryTemplateForm({ templateId, onSuccess }: EnhancedStoryTemplateFormProps) {
  ...
  const createTemplateMutation = useMutation({
    mutationFn: async (data: InsertVideoTemplate) => {
      return apiRequest("/api/video-templates", {
        method: "POST",
        body: JSON.stringify({
          ...data,
    ...
    onSuccess: () => {
      toast({ title: "Video template created successfully" });
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="e.g., Ghost Stories, True Crime, etc." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <TabsContent value="content" className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Manual list of ideas for video generation, separated by delimiter
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-4 gap-4">
                    <div className="col-span-3">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Ideas (one per line or separated by delimiter)</FormLabel>
                            <FormControl>
                              <Textarea 
                        ...
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="flux-pro">Flux Pro (High Quality)</SelectItem>
                          ...
                          <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                            ...
                            <div className="space-y-1 leading-none">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Use higher quality model for first and last images
                              </p>
                            </div>
                          </FormItem>
                        )}
            ...
            <TabsContent value="audio" className="space-y-6">
              <Card>
                ...
                <CardContent className="space-y-6">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            <SelectItem value="eleven_labs">ElevenLabs (Premium)</SelectItem>
                  ...
                  <div className="space-y-4">
                    <FormLabel>Voice Selection (Multiple voices will rotate per video)</FormLabel>
                      ...
                      {VOICE_OPTIONS.map((voice) => (
                        <div key={voice.id} className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedVoices([...selectedVoices, voice.id]);
                              } else {
                                setSelectedVoices(selectedVoices.filter(v => v !== voice.id));
                              }
                            }}
                            className="rounded"
                          />
                          <label htmlFor={voice.id} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                            {voice.name}
                          </label>
                        </div>
                      ))}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Pause Between Audio Clips: {field.value}ms</FormLabel>
                        <FormControl>
                          <Slider
                            min={100}
                            max={2000}
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Transition Duration: {field.value}s</FormLabel>
                          <FormControl>
                            <Slider
                              min={0.5}
                              max={5}
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            form.setValue('videoEffects', { ...effects, kenBurns: checked });
                          }}
                        />
                        <label className="text-sm font-medium">Enable Ken Burns zoom effect</label>
                      </div>

                      {form.watch('videoEffects')?.kenBurns && (
                        <div className="grid grid-cols-2 gap-4 pl-6">
                          <div>
                            <label className="text-sm font-medium">Speed: {form.watch('videoEffects')?.kenBurnsSpeed}</label>
                            <Slider
                              min={0.5}
                              max={3}
                              step={0.1}
                              value={[form.watch('videoEffects')?.kenBurnsSpeed || 1]}
                              onValueChange={(value) => {
                                const effects = form.getValues('videoEffects') || {};
                                form.setValue('videoEffects', { ...effects, kenBurnsSpeed: value[0] });
                              }}
                            />
                          </div>
                          <div>
                            ...
                            <Select
                              value={form.watch('videoEffects')?.kenBurnsDirection}
                              onValueChange={(value) => {
                                const effects = form.getValues('videoEffects') || {};
                                form.setValue('videoEffects', { ...effects, kenBurnsDirection: value });
                              }}
                            >
                              <SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          const effects = form.getValues('videoEffects') || {};
                          form.setValue('videoEffects', { ...effects, filmGrain: checked });
                        }}
                      />
                      <label className="text-sm font-medium">Film Grain Effect</label>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          const effects = form.getValues('videoEffects') || {};
                          form.setValue('videoEffects', { ...effects, fog: checked });
                        }}
                      />
                      <label className="text-sm font-medium">Fog/Atmospheric Effect</label>
                    </div>
                  </div>
                </CardContent>
              </Card>
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          <FormLabel className="text-base">Enable Captions</FormLabel>
                          <p className="text-sm text-muted-foreground">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Words Per Caption: {field.value}</FormLabel>
                              <FormControl>
                                <Slider
                                  min={1}
                                  max={8}
